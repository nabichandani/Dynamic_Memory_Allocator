...
Test(sf_memsuite_student, malloc_splinter, .init = sf_mem_init, .fini = sf_mem_fini) {
	...
	sf_header *header = (sf_header*)((char*)y - 8);
	cr_assert(header->allocated == 1, "Allocated bit is not set!");
	cr_assert(header->padded == 1, "Padded bit not set for splinter!");
	cr_assert(header->block_size << 4 == 4048, "Splinter not added!");
	cr_assert(fl->head == NULL, "Free Block size not what was expected!");

	sf_realloc(y, 0);
	cr_assert(header->allocated == 0, "Allocated bit is not removed!");
	cr_assert(fl->head->header.block_size << 4 == 4048, "Free Block size not what was expected!");
}

Test(sf_memsuite_student, over_mem_limit, .init = sf_mem_init, .fini = sf_mem_fini) {
	sf_malloc(4092);
	void* y = sf_malloc(4092);
	void* z = sf_malloc(4092);
	void* w = sf_malloc(4092);

	cr_assert(w == NULL, "w is not NULL");
	cr_assert(sf_errno == ENOMEM, "errno not set");

	sf_free(z);//coalesces with remaining free block
	sf_realloc(y, 0); // coalesces with the rest

	free_list *fl = &seg_free_list[find_list_index_from_size(12272)];
	cr_assert(fl->head->header.block_size << 4 == 12272, "Free Block size not what was expected!");

}

Test(sf_memsuite_student, malloc_then_realloc_larger, .init = sf_mem_init, .fini = sf_mem_fini) {
	void* x =sf_malloc(1);
	void* y = sf_realloc(x,4048);

	free_list *fl = &seg_free_list[find_list_index_from_size(32)];
	cr_assert(fl->head->header.block_size << 4 == 32, "Free Block size not what was expected!");
	cr_assert(y != x, "The pointer to x is equal to y!");

	sf_header *head = (sf_header*)((char*)y - 8);
	cr_assert(head->block_size << 4 == 4064, "Block size for Y is wrong.");
	cr_assert(head->padded == 0, "Padding for Y is wrong.");
	cr_assert(head->allocated == 1, "Alloc for Y is wrong.");

	sf_header *head2 = (sf_header*)((char*)x - 8);
	cr_assert(head2->padded == 0, "Padding for X is wrong.");
	cr_assert(head2->allocated == 0, "Alloc for X is wrong.");
}

Test(sf_memsuite_student, malloc_3_pages, .init = sf_mem_init, .fini = sf_mem_fini) {
	void* x =sf_malloc(12272);
	sf_header *head = (sf_header*)((char*)x - 8);
	cr_assert(head->block_size << 4 == 12288, "Block size is wrong.");
	cr_assert(head->padded == 0, "Padding is wrong.");
	cr_assert(head->allocated == 1, "Alloc is wrong.");

	sf_free(x);
	free_list *fl = &seg_free_list[find_list_index_from_size(12288)];
	cr_assert(fl->head->header.block_size << 4 == 12288, "Free Block size not what was expected!");
}

Test(sf_memsuite_student, splinter_realloc_smaller, .init = sf_mem_init, .fini = sf_mem_fini) {
	void* x =sf_malloc(4016);
	void* y = sf_realloc(x, 4000);

	sf_header *head = (sf_header*)((char*)y - 8);
	cr_assert(y == x, "The pointer to x is equal to y!");
	cr_assert(head->padded = 1, "Padded for realloc val is not seeing splinter!");
	cr_assert(head->block_size<<4 == 4032, "Block Size is not correct!");

	free_list *fl = &seg_free_list[find_list_index_from_size(64)];
	cr_assert(fl->head->header.block_size << 4 == 64, "Free Block size not what was expected!");

	sf_realloc(y, 0);
	free_list *fl2 = &seg_free_list[find_list_index_from_size(4096)];
	cr_assert(fl2->head->header.block_size << 4 == 4096, "Free Block size not what was expected!");
}
